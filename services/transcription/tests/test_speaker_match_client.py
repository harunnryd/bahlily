...
@pytest.mark.asyncio
async def test_match_bulk_returns_empty_dict_on_missing_response_key(
    respx_mock: respx.MockRouter,
) -> None:
    respx_mock.post("https://storage/speaker-profiles/match-bulk").mock(
        return_value=httpx.Response(
            200,
            json={"matches": [{"key": "S1", "profile": {"id": "p1", "name": "Alice"}}]},
        )
    )
    client = SpeakerMatchClient(storage_url="https://storage")
    assert await client.match_bulk([("S1", [0.0] * 512), ("S2", [1.0] * 512)]) == {}
